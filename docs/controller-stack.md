...
The current layer is **normative artifacts**. It adds production-owned
registries, deterministic generators, fixed vectors, generated schemas, and
quarantined configuration on top of the dormant composition. Generated bytes
are review artifacts, not an activation mechanism. The layer intentionally has
no provider client, network transport, subprocess call, workflow dispatch,
secret, OIDC permission, artifact upload, or target-repository route.
