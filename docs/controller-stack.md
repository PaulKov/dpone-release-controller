...
The current layer is **controller composition**. It composes the earlier pure
building blocks into an offline operation state machine, typed route catalog,
receipt chain, candidate exchange, and wire registry. No composition is wired
to a runtime entry point. The layer intentionally has no provider client,
network transport, subprocess call, workflow dispatch, secret, OIDC permission,
artifact upload, or target-repository route.
