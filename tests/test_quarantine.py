...
class WorkflowQuarantineTests(unittest.TestCase):
    ...
    def test_required_quarantine_check_aggregates_all_ci_authorities(self) -> None:
        """Keep the stable check fail-closed across controller and broker CI."""
        ...
        self.assertIn("    needs:\n      - contract\n      - broker\n", text)
        self.assertIn("          BROKER_RESULT: ${{ needs.broker.result }}\n", text)
        self.assertIn("          CONTRACT_RESULT: ${{ needs.contract.result }}\n", text)
        self.assertIn('          test "${BROKER_RESULT}" = "success"\n', text)
