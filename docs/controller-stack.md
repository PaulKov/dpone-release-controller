...
The current layer is **state and service primitives**. It adds pure operation,
route, service-inventory, activation-proof, receipt-payload, and preflight
building blocks on top of the domain layer. None is wired to a runtime entry
point. The layer intentionally has no provider client, network transport,
subprocess call, workflow dispatch, secret, OIDC permission, artifact upload,
or target-repository route.
